...
public class AfterUserOrderDaoImpl {
     public List QueryOrderlist(){
    	 ...
    	 String sql="SELECT o.order_id,a.rec_name,o.orderdate,o.carcount,o.orderprice,case state WHEN 0 then '未付款' else '已付款' END state ,case orderstate WHEN 0 then '未发货' else '已发货' END orderstate from dt_goods_order AS o LEFT JOIN dt_address a on o.address_id =a.address_id ";
    	 ...
    	 if(list1.size()>0){ 
  		  ...
  		  order.setReman(String.valueOf(map.get("rec_name")));
  		  order.setCommname(String.valueOf(map.get("orderdate")));
     ...
     public PageModel QueryUser(String page,String rows){
 		List<User> list=null;
 		String sql1 = "select user_id,username,password,case sex when 0 then '女' else '男' end sex,realname,birthday,email,telphone,picture,case state when 0 then '失效' else '正在使用' end state from dt_user  ";
 		PageModel pageModel=null;
 		ArrayList<HashMap<String, Object>> list1 = odbcUtils.executeQuery(sql1, null);
 	...
 	public PageModel QueryUserCondition(String id,String page,String rows){
 		...
 		pageModel.setList(list);
 		ArrayList<HashMap<String,Object>>  myList=null;
 		myList=  odbcUtils.executeQuery(pageModel.toCountSql(),new String[]{id} );
 		if(myList.size()>0){
			HashMap  map  =myList.get(0);
			Integer  count =Integer.valueOf(String.valueOf(map.get("count")));
			//把条数放到模型中1
 	...
 	public List QueryOrderCondition(Object []obj){
 		List<Order> list=null;
 		String sql = "SELECT o.order_id,a.rec_name,o.orderdate,o.carcount,o.orderprice,case state WHEN 0 then '未付款' else '已付款' END state ,case orderstate WHEN 0 then '未发货' else '已发货' END orderstate from dt_goods_order AS o LEFT JOIN dt_address a on o.address_id =a.address_id where o.order_id=? || o.state=? || o.orderstate=?";
